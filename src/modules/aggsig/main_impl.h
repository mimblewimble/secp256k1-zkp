...
int secp256k1_aggsig_sign_single_extra(const secp256k1_context* ctx,
    unsigned char *sig64,
    const unsigned char *msg32,
    const unsigned char *seckey32,
    const unsigned char* secnonce32,
    ...
    /* add extra scalar */
    secp256k1_scalar_set_b32(&extra, extra32, &overflow);
    if (overflow) {
        secp256k1_scalar_clear(&sec);
        return 0;
    }
    secp256k1_scalar_add(&sec, &sec, &extra);
...
int secp256k1_aggsig_verify_single_extra(
    ...
    secp256k1_pubkey_load(ctx, &extra_ge, extrakey);
    secp256k1_ge_neg(&extra_ge, &extra_ge);
    secp256k1_gej_add_ge(&pk_sum, &pk_sum, &extra_ge);
